(function(GLOBAL) {
	...
	//
	// Minor modifications made by Konstantin Marmaluykov(kmarmaliykov@exadel.com) 
	...
	function phantomTouchEnd(e) {
		...
		fingers.forEach(function(finger) {
			...
			var x = finger.x, y = finger.y;
			var elementMouseIsOver = document.elementFromPoint(x, y);
			if (elementMouseIsOver.tagName && elementMouseIsOver.tagName.toLowerCase() == "input") {
				elementMouseIsOver.select();
			}
			// Mobile Safari moves all the mouse event to fire after the touchend event.
				...
				if (finger.target === elementMouseIsOver) {
	...
	if (config.startOnLoad) setTimeout(start,0);
}(this));
